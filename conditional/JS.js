...
//Create Scales
var xScaleCP = d3.scale.linear().domain([0, 1]).range([0,710]);
var xWidthCP = d3.scale.linear().domain([0, 1]).range([0,710]);
var yScaleCP = d3.scale.linear().domain([0, 1]).range([0,460]);
...
var rects1 = events.append('rect').attr('class', function(d){ return (d.name + ' shelf') }).call(dragRect);
...
function updateRects(dur) {
  rects1.transition().duration(dur)
...
svgBallCP.append("rect")
	.attr('x',20).attr('y',0)
	.attr('width',xScaleCP(1)).attr('height',113+23)
	.attr('fill',"white").moveToBack();
var light1=svgBallCP.append("g").moveToBack().selectAll("rect")
    .data(lightrays)
    .enter().append("rect");
function addBars(){
	light1
	.attr("width", 1)
    .attr("x", function(d,i) { return d%710+20; })
    .attr("y", function(d,i) { return 92*Math.floor(d/710)+136})
    .attr("height", 92)
    .attr('fill',function(d,i) {
    	var x=d%710;
    	var layer=Math.floor(d/710)+1;
    	var p=xScaleCP.invert(x);
    	var a=(eventsData[0].x <= p && p <= eventsData[0].x + eventsData[0].width);
    	var b=(eventsData[1].x <= p && p <= eventsData[1].x + eventsData[1].width);
    	var c=(eventsData[2].x <= p && p <= eventsData[2].x + eventsData[2].width);
    	if(layer==1){
			if(a){
				return "orange";
			}
